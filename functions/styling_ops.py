...
from tkinter import font, TclError, Toplevel, StringVar, IntVar, Label, Button, Spinbox, Text
from tkinter.simpledialog import askstring, askinteger
from tkinter.ttk import Combobox

def set_font(TextArea, app=None):
    """Let user pick font family and size."""
    # Get current font
    current_font = TextArea.cget("font").split()
    family = askstring("Font Family", "Enter font family (e.g., Arial, Courier):", initialvalue=current_font[0])
    size = askinteger("Font Size", "Enter font size:", initialvalue=int(current_font[1]))

    if family and size:
        TextArea.config(font=(family, size))
        if app:
            app.current_font = (family, size)

def _toggle_style(TextArea, style_tag, style_config, app=None):
    """Helper to toggle a tag (bold/italic/underline) on selected text."""
    try:
...
def toggle_underline(TextArea, app=None):
    """Toggle underline style on selection."""
    if app and hasattr(app, "current_font"):
        family, size = app.current_font
    else:
        family, size = ("Arial", 13)
...
def set_font(TextArea, app=None):
    ...
    preview.config(state="disabled")  # Read-only
